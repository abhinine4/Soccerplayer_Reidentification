...
from torchreid.models.resnet import Bottleneck
...
def visactmap(
    ...
    for target in list(test_loader.keys()):
        ...
        for batch_idx, data in enumerate(data_loader):
            ...
            try:
                l_outputs = model(imgs, return_featuremaps=True, return_activations=True)
            except TypeError:
                raise TypeError(
            ...
            # compute activation maps for all the layers

            for l in range(0, len(l_outputs)):
                outputs = l_outputs[l]
                ...
                for j in range(outputs.size(0)):
                    ...
                    cv2.imwrite(osp.join(actmap_dir, imname + 'l' + str(l) + '.jpg'), grid_img)
            ...
            cnt += 1
            # We don't need the activation map of all the images, but only #cnt
            if cnt >= 6:
                break
...
# we probably can use already given function above, so have not completed this one
'''
def visualize_activations(model_path, image_path):
    ...
    for child in model_children:
        if type(child) == nn.Conv2d:
            ...
            for layer in child.children():
                ...
                elif type(layer) == Bottleneck:
                    for llayer in layer.children():
                        if type(llayer) == nn.Conv2d:
                            no_of_layers += 1
                            conv_layers.append(llayer)
    print(no_of_layers)
    print(conv_layers)
'''
