...
class ResNet(nn.Module):
    ...
    def featuremaps(self, x, return_featuremaps=False, return_activations=False):
        featuremaps = []
        x = self.conv1(x)
        featuremaps.append(x.clone().detach())
        x = self.bn1(x)
        featuremaps.append(x.clone().detach())
        x = self.relu(x)
        # featuremaps.append(x.clone().detach())
        ...
        if self.fc is not None and not return_activations:
            x = self.fc(x)
    ...
    def forward(self, x, return_featuremaps=False, return_activations=False):
        # return maps used only from the actmap() function.

        # modified for layer-wise similarity
        return_featuremaps = True

        f = self.featuremaps(x, return_featuremaps, return_activations)

        if return_featuremaps and return_activations:
            return f

        if not self.training:
            return f[-1]

        y = self.classifier(f[-1])
